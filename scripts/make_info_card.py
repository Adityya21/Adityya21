...
ROWS = [
    ("host",),
    ("kv", "Now", "Final Year Student"),
    ("kv", "Focus", "Data Science"),
    ("kv", "Edu", "BTech CE, NMIMS'27"),
    ("kv", "Location", "Mumbai, India"),
    ("gap",),
    ("sec", "Stack"),
    ("kv", "Languages", "Python, C++, SQL"),
    ("kv", "Web", "React, HTML/CSS, Node.js"),
    ("kv", "Data Vis", "Matplotlib, NumPy, Pandas, Seaborn, Power BI"),
    ("kv", "ML & Stats", "Scikit-learn, TensorFlow, XGBoost, LangChain"),
    ("kv", "Tools", "Git, Jupyter"),
]
...
parts.append(f'<text x="{W/2}" y="{TITLEBAR_H/2 + 4}" fill="{MUTED}" font-size="12" '
             f'text-anchor="middle">aditya@github: ~$ neofetch</text>')
...
for i, row in enumerate(ROWS):
    ...
    if kind == "host":
        inner = (f'<text x="{KEY_X}" y="{y:.1f}" font-size="14" font-weight="700">'
                 f'<tspan fill="{GREEN}">aditya</tspan><tspan fill="{MUTED}">@</tspan>'
